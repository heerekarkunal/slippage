...
BCL_TOOLS = {
    "bcl-convert": "Download from https://support.illumina.com/sequencing/sequencing_software/bcl-convert.html",
}
...
def verify_dependencies(aligner: str, logger, require_bcl: bool = False):
    missing = []
    ...
    if require_bcl:
        if _check_system_tool("bcl-convert"):
            logger.debug("  ✓ bcl-convert")
        else:
            hint = BCL_TOOLS["bcl-convert"]
            logger.error(f"  ✗ bcl-convert missing  →  {hint}")
            missing.append("bcl-convert")
    if missing:
        logger.error("")
